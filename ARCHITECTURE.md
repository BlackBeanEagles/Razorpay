...
**Several real bugs were found and fixed here** (not hypothetical — concurrency ones reproduced with `threading.Barrier` to force genuinely simultaneous execution; see README.md's "Security review" section for the full list across the whole codebase):
...
3. **Non-positive amount accepted**: `_validate_mandate_data` checked expiry, single-use, and the spend ceiling, but never that `requested_amount_inr` was itself positive — a negative amount made recorded spend go *down* instead of up, permanently growing a mandate's remaining headroom (and defeating `single_use`) instead of consuming it. Now rejected unconditionally before any other check runs.
4. **Idempotency check leaking cross-customer data**: the same fast-path that fixed race #2 matched a prior successful order by `razorpay_order_id` alone, with no ownership check — a different customer who merely knew or guessed a real order id could retrieve another customer's real verification data. Now scoped to `requesting_customer_id`.
...
- An AI buyer gets its own identity namespace (`ai_buyer_<random hex>`, never overlapping with human `cNNN` customer IDs) — so the audit trail can always tell an autonomous purchase from a human one at a glance. Ids are `secrets.token_hex(8)`-based, not sequential: an earlier version minted `ai_buyer_001`, `ai_buyer_002`, ... which let any MCP caller — nothing authenticates the `ai_buyer_id` argument itself — enumerate every registered buyer and probe their loyalty-tier/discount data without ever having registered as them.
...
- `purchase()` calls `check_price_fairness` itself and refuses a flagged price before ever reaching Guardrail. This server's own `instructions` string *asks* a connecting agent to call `check_price_fairness` first, but that's advisory text an external agent can simply not follow — an earlier version enforced nothing else, so `purchase(product_id="laptop_premium", amount_inr=1, ...)` would succeed outright for any mandate with ≥1 INR of headroom. The storefront's own chat agent (`agent/llm_agent.py`) had the identical gap (its system prompt, not code, was the only thing asking the LLM to check first) and got the same fix.
...
`MAX_ROUNDS` was, until a later security review, enforced only against the caller's own `round_number` argument — a caller that always sent `round_number=1` got an uncapped number of rounds, since nothing persisted how many times a given `(product_id, customer_id)` pair had actually negotiated. `propose_price` now tracks a small persisted attempt counter per pair (`negotiation_rounds.json`) and enforces `max(caller's round_number, server-tracked count)`, so the cap holds even against a caller that never increments its own count.
...
`record_dispute_created()` used to unconditionally overwrite any existing record for a `dispute_id` back to `status: "draft_pending"` — a real gap, since Razorpay redelivers webhooks on slow/ambiguous responses, and a redelivery arriving after an admin had already submitted the response would silently revert the local record and wipe who submitted it and when. It now checks the existing record's status first and treats a redelivery for anything past `draft_pending` as a no-op.
...
- **Rate limiting** (`api/rate_limit.py`): 5 failed login attempts within 5 minutes locks that key out for 5 minutes, persisted to disk and cross-process-locked — a purely in-memory limiter would let an attacker's requests spread across multiple worker processes each get their own independent budget. The check (`seconds_until_unlocked`) and the write (`record_failure`) used to be two separately-locked steps with the ~100ms PBKDF2 hash unlocked in between them — a real concurrency race a later security review found: a burst of simultaneous login attempts could all read the same under-the-limit count before any of them committed a failure, blowing past the 5-attempt cap. `reserve_attempt()` now checks and records as one atomic operation before the hash runs.
- **Cross-process file locking** (`api/file_lock.py`): atomic lockfile creation (`O_CREAT|O_EXCL`, safe on both Windows and POSIX) shared by the mandate store, rate limiter, and session revocation list — a plain `threading.Lock` only protects within one process, not across `--workers N`. Guardrail originally carried its own independent second copy of this exact technique, which had already drifted (this module gained a stale-lock PID-liveness check the other lacked, so a lock reclaimed purely by file age — no check the holder was actually dead — could be stolen from a holder still genuinely running under heavy contention). Consolidated into this one shared implementation; `guardrail.py` now imports it directly.
- **Webhook payload robustness** (`api/routes/webhooks.py`): a validly-signed but structurally malformed payload (a non-object JSON body, or a key present with value `null`) used to crash the route with an unhandled 500 instead of the same honest, non-retried 4xx-style response a signature failure already gets. Now caught centrally around the event-type dispatch.
- **Price fairness enforced in code, not just asked for in a prompt**: both the MCP server's `purchase()` tool and the storefront LLM agent's `execute_purchase` tool handler now call `check_price_fairness` themselves and refuse a flagged price before ever reaching Guardrail — previously this was only requested via the MCP server's `instructions` string / the LLM's system prompt, which a non-compliant or prompt-injected caller could simply not follow.
...
All of the above marked "a later security review found" were discovered and fixed in one dedicated pass over the money-critical paths (Guardrail, auth, webhooks, MCP), each landed as its own commit — see README.md's "Security review: what broke, and what we did about it" for the full list with exploit scenarios, and `git log` for the fix-by-fix detail.

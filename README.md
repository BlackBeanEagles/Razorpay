...
- **Live verification** (`reconciliation/live_verification.py`) — goes one step further than reconciling this app's own files against each other: for every real captured purchase, it re-fetches that order's actual current payment record straight from Razorpay's real test-mode API (`guardrail/razorpay_rest.py`, the same client the live checkout flow already uses) and compares it to what our ledger expected, *right now*, not at confirm-time. Catches drift the ledger-vs-ledger check structurally cannot see — e.g. a refund issued by hand straight in the Razorpay Dashboard, bypassing this app entirely. One exception type (`overcharge_drift` — Razorpay shows more captured than was ever meant to be charged) is unambiguous enough to remediate automatically: the dashboard's "Refund overcharge" button re-verifies fresh against Razorpay immediately before acting (never trusts a stale check) and issues a real test-mode refund for exactly the difference. Every other drift type is flagged for manual review only — this system never auto-refunds an *undercharge* or re-charges a customer, since the right fix depends on context it has no way to know. Exposed read-only to Settlement Q&A too (`get_live_drift_check`); the LLM agent can report a remediable exception but is never allowed to trigger the refund itself.
- **Bank-registered allowance top-ups** (`subscriptions/allowance_subscription.py`) — a customer can optionally register a real, recurring Razorpay Subscription (`POST /v1/plans` + `POST /v1/subscriptions`, plain self-serve test-mode REST) that refreshes one of their own Guardrail mandates every month, authorized via UPI AutoPay/eMandate/card at Razorpay's own real hosted checkout. Scoped honestly to what a Subscription actually is — a fixed-amount, fixed-schedule recurring charge the customer's bank independently confirms — not a claim that Guardrail's ad-hoc spend-triggering itself is bank-enforced (that would need Razorpay's Recurring Payments API, which is gated behind a support-activated request, not self-serve). The refresh itself only ever happens from a real, signature-verified `subscription.charged` webhook: `guardrail.renew_mandate()` resets the mandate's spend counter to 0 and moves its expiry to the real, bank-confirmed billing cycle's own `current_end` — the ceiling itself (`max_amount_inr`) never grows, so a bug in this app's own server still can't get the customer's AI agent more spending room than their bank actually authorized. See `subscriptions/_demo_allowance_subscription.py` for a live, real-API demo driver.
  - **Recovery** — mirrors Razorpay's own "Subscription Recovery" Agent Studio pattern: when Razorpay exhausts its own retries and halts a subscription, `send_recovery_link()` fires automatically (a real, one-time `POST /v1/payment_links` Payment Link, emailed via Resend) so the customer can fix it in one click instead of silently staying blocked until the next cycle. A real `payment_link.paid` webhook then renews the mandate the same way a normal charge would. Dashboard's "Allowance subscriptions" panel shows every subscription and lets an admin manually resend a recovery link too.
- **Bank statement cross-check** (`reconciliation/bank_statement.py`) — a third reconciliation source, independent of both this app's own ledger and Razorpay's own API: an admin pastes real bank statement text (CSV rows or copy-pasted lines) on the dashboard, a forced Groq tool call extracts structured transactions, and they're matched against the real ledger by amount and date — never by UTR, since test mode generates no real one on our side (a live-mode account gets exact UTR matching for free). Directly inspired by Razorpay's own Agentic Platform ("Intelligent Reconciliation": upload a bank statement, an agent cross-references it against Razorpay records), scoped here to pasted text rather than OCR'd screenshots.
- **Disputes & chargebacks** (`disputes/dispute_response.py`) — when Razorpay tells us via a real, signature-verified `payment.dispute.created` webhook that a customer's bank has raised a chargeback, evidence is drafted automatically from this system's own real audit trail (the exact Guardrail verification record and Parity fairness check for that order) and saved as a real draft on Razorpay's side (`PATCH /v1/disputes/:id/contest`, `action: "draft"` — Razorpay's own documented behavior never auto-submits a draft). Nothing here ever calls `action: "submit"` or `POST /v1/disputes/:id/accept` on its own; those are the two dispute-committing actions and stay a deliberate, confirmed click on the dashboard's "Disputes & chargebacks" panel. `payment.dispute.{won,lost,closed,under_review,action_required}` webhooks keep the local record in sync with Razorpay's real outcome. See "Demoing the disputes feature" below for how to actually show this working.
...
- **Negotiation** (`negotiation/negotiation.py`) — real agent-to-agent price haggling: an AI buyer proposes a price, and the merchant's own pricing agent accepts, counters (with the real fairness-bounded floor, computed from the exact same logic `check_price_fairness` uses), or rejects after a few rounds. Exposed identically to the MCP server and the storefront's own LLM agent. Verified live over the real MCP protocol (`mcp_server/_demo_negotiate.py`).
...
- **API** (`api/`) — FastAPI wrapper exposing the pipeline over HTTP and serving the frontend. Key routes: `/api/chat[/stream]`, `/api/mandates`, `/api/catalog`, `/api/purchase/{initiate,confirm}`, `/api/webhooks/razorpay`, `/api/auth/*`, `/api/customer/*`, `/api/support-request[s]`, and the admin-only `/api/audit-log`, `/api/live-stats`, `/api/batch-results`, `/api/ai-agents`, `/api/settlement-qa`, `/api/reconciliation/live-check`, `/api/reconciliation/remediate-overcharge`, `/api/reconciliation/bank-statement`, `/api/disputes`, `/api/disputes/{id}/{submit,accept}`, `/api/allowance-subscriptions[/for-mandate/{id}]`, `/api/admin/allowance-subscriptions[/{id}/send-recovery-link]`.
- **Payment webhook** (`api/routes/webhooks.py`) — Guardrail previously only learned a real Checkout payment succeeded because the browser itself called `/api/purchase/confirm`; if that tab closed right after paying, the payment was genuinely captured but never recorded. `POST /api/webhooks/razorpay` closes that gap with a real, HMAC-signature-verified `payment.captured` webhook that calls the exact same `confirm_purchase()` the frontend path calls — never trusted on its own, always independently re-verified with Razorpay. Needs `RAZORPAY_WEBHOOK_SECRET` in `.env` and a public HTTPS URL pointed at this server (ngrok, Razorpay CLI forwarding, or a real deployment) to actually receive events — Razorpay can't reach `localhost`.
...
## Demoing the disputes feature

Razorpay has no self-serve "create a test dispute" endpoint (real disputes are bank/issuer-initiated), so there's no way to trigger a genuinely real one on demand for a recording. What's real and demoable instead:

1. Set `RAZORPAY_WEBHOOK_SECRET` in `.env` (see that file's comments) and start the app.
2. Complete **one** real purchase through the storefront — pay with one of Razorpay's own domestic test cards (e.g. `4100 2800 0000 1007`, any future expiry, any CVV, any 4-10 digit OTP — see [Razorpay's test card docs](https://razorpay.com/docs/payments/payments/test-card-details/); a generic international-looking test number like `4111 1111 1111 1111` gets a real, correct "international cards are not supported" rejection from Razorpay's gateway, since it isn't one of their own domestic test BINs) at real Checkout, and let it confirm. This puts one genuine order into `guardrail/ledger.json`.
3. Run `python disputes/_demo_dispute.py` (no arguments needed — it finds that order automatically). This sends a **real HTTP POST**, with a **real HMAC signature**, to your own running server's `/api/webhooks/razorpay` — exactly the request shape Razorpay itself would deliver, just triggered manually instead of by an actual bank dispute.
4. Watch the dashboard's "Disputes & chargebacks" panel — the evidence shown there was genuinely drafted from that order's real Guardrail verification record and Parity fairness check, not fabricated for the demo.
5. The script also attempts the real `PATCH /v1/disputes/:id/contest` call against Razorpay. Since the dispute id is synthetic (no real dispute exists to attach it to), Razorpay correctly returns a real 404 — shown honestly in the terminal output and on the drafted record, not hidden. That 404 is itself worth showing: it's proof this isn't faking a success response, it's a real call to Razorpay's real API failing for the one reason it's expected to.
6. Clicking "Submit response" or "Accept & refund" on the dashboard will hit the same real endpoint and 404 the same honest way, for the same reason — a legitimate limitation of demoing against a synthetic dispute in test mode, not a bug.

## Demoing the bank-registered allowance top-up

1. Log in as a customer, set a personal spending limit (this becomes your own owned mandate, not the shared demo one), and the mandate box shows "Set up automatic monthly refresh via UPI AutoPay".
2. Clicking it creates a **real** test-mode Razorpay Plan + Subscription and opens Razorpay's own hosted checkout (`short_url`) in a new tab — completing that with a real UPI app or Razorpay's published test card is the actual bank-side authorization step; nothing here fakes it.
3. To show the refresh itself without waiting a real month: run `python subscriptions/_demo_allowance_subscription.py` (needs `RAZORPAY_WEBHOOK_SECRET` in `.env` and a running server, same as the disputes demo). It creates a real mandate + real Plan/Subscription, then sends a real, signed `subscription.charged` webhook — and prints the mandate's spend counter genuinely resetting to 0 as a direct result, not simulated.
...
python batch_tests/run_negotiation_batch.py
```
...
- **Negotiation:** 6 scripted buyer scenarios, 5 reach a real agreement, 1 never does (deliberately unreasonable, proving it doesn't concede forever) — 0 fairness-floor violations across every accepted deal.
- **Live verification:** no batch script — by design, it has nothing synthetic to run against. It only ever checks real captured orders in `guardrail/ledger.json` against Razorpay's real API, so its correctness is proven entirely by `reconciliation/test_live_verification.py`'s 16 tests (every drift type and the remediation guardrails, each against a monkeypatched Razorpay response) rather than a scored batch run. The dashboard's "Live drift check" panel reports "0 real purchases to check" until at least one real Checkout has actually been completed.
